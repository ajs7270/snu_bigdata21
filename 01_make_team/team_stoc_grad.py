...
# 두 학생을 swap 하여 loss를 줄이는 방식
# 확률적으로 loss가 높아지는 방향으로도 이동
num_of_member = 4
...
for i in range(300000000):
    ...
    elif random.random() < 0.0000005:
        # loss가 올라가더라도 확률적으로 교체를 함 -> local optima에서 빠져나올 수 있음
        result = swap_teams
        cur_loss = swap_loss
        print(i, "th iteration :", cur_loss)

    # 가장 작은 loss를 저장해 놓음
    if global_loss > cur_loss:
        global_result = result
        global_loss = cur_loss

print("=====================")
print("global loss :", global_loss)
for idx, team in enumerate(global_result):
    print("Team", idx + 1)
    for student in team:
        print(student)